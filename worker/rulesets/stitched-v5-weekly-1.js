...
function deserializeEndlessRun(serialized) {
  try {
    ...
    if (!record(payload) || payload.version !== 1 && payload.version !== 2 && payload.version !== 3 && payload.version !== 4 && payload.version !== 5 || !validWorld(payload.run, payload.version) || !record(payload.run)) return null;
...
function createEndlessRun(seed, generationVersion = 5) {
...
function validateToolUse(run, candidate) {
  ...
  if (run.generationVersion !== 5) return false;
...
function applyEndlessTool(run, use, paid = false) {
  ...
  if (!paid && !(run.inventory[use.tool] > 0)) return false;
  if (!paid) run.inventory[use.tool] -= 1;
...
function createRankedSimulation(seed) {
  return createEndlessRun(seed, 5);
}
