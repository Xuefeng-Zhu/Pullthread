  ...
  for (let count = 0; count < MAX_CONTACTS_PER_STEP && remaining > EPSILON; count += 1) {
    ...
    if (contact.kind === "barrier") {
      ...
      if (barrier2.kind === "tearable" && -incoming >= 400) {
        ...
        if (Math.abs(normal.x) < EPSILON && Math.abs(normal.y) > 1 - EPSILON && outwardSpeed < BARRIER_RESTING_NORMAL_SPEED && Math.abs(state.velocity.x) < BARRIER_ESCAPE_SPEED) {
          const middle = barrier2.x + barrier2.width / 2;
          state.velocity.x = state.position.x < middle ? -BARRIER_ESCAPE_SPEED : BARRIER_ESCAPE_SPEED;
        }
