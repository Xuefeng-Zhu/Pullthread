-- RevenueCat Test Store purchases use the existing transaction and FIFO
-- wallet-lot pipeline. They are accepted by application logic only in sandbox.
CREATE TABLE commerce_lots_test_store_backup AS SELECT uid, environment, tx_key, remaining, purchased_at, active, revoked FROM commerce_lots;
DROP TABLE commerce_lots;
CREATE TABLE commerce_transactions_v6 (tx_key TEXT PRIMARY KEY NOT NULL, uid TEXT NOT NULL, environment TEXT NOT NULL CHECK(environment IN ('sandbox','production')), transaction_id TEXT NOT NULL, product_id TEXT NOT NULL, store TEXT NOT NULL CHECK(store IN ('app_store','play_store','rc_billing','stripe','paddle','test_store')), quantity INTEGER NOT NULL CHECK(quantity BETWEEN 1 AND 100), purchased_at INTEGER NOT NULL, refunded INTEGER NOT NULL CHECK(refunded IN (0,1)), points INTEGER NOT NULL CHECK(points > 0), updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(uid, environment) REFERENCES commerce_wallets(uid, environment));
INSERT INTO commerce_transactions_v6 (tx_key, uid, environment, transaction_id, product_id, store, quantity, purchased_at, refunded, points, updated_at) SELECT tx_key, uid, environment, transaction_id, product_id, store, quantity, purchased_at, refunded, points, updated_at FROM commerce_transactions;
DROP TABLE commerce_transactions;
ALTER TABLE commerce_transactions_v6 RENAME TO commerce_transactions;
...
INSERT INTO commerce_lots SELECT uid, environment, tx_key, remaining, purchased_at, active, revoked FROM commerce_lots_test_store_backup;
DROP TABLE commerce_lots_test_store_backup;
