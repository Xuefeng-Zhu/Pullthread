...
import { chooseSection } from './sections';
import { chooseWorldSection, INTRODUCED_MECHANICS, type WorldSectionPattern } from './worldSections';
...
export function updateSectionWindow(run: EndlessRun): void {
  ...
  while (run.nextPocketIndex <= run.highestPocket + 5) {
    ...
    const pattern = run.generationVersion === 3
      ? chooseWorldSection(run.seed, index, run.lastExitX, progress.lastFamily, run.pocketsCaught, progress.introductions!)
      : chooseSection(run.seed, index, run.lastExitX, progress.lastFamily, run.pocketsCaught >= 12);
    const worldPattern = run.generationVersion === 3 ? pattern as WorldSectionPattern : undefined;
    ...
    const section: OwnedSection = {
      ...
      ...(worldPattern ? { worldStage: worldStageForScore(run.pocketsCaught),
        mechanics: worldPattern.mechanics, windZoneIds: ownedWindZones.map((zone) => zone.id),
  ...
  const windZoneIds = new Set(progress.sections.flatMap((section) => section.windZoneIds ?? []));
