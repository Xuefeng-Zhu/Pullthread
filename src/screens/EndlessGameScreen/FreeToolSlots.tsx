...
export function FreeToolSlots({ queue, highContrast = false, testID = 'free-tool-slots', toolTestIDPrefix, onTool, isUnavailable, ordered = true, compact = false, micro = false }: {
  ...
  return <View testID={testID} accessible={!onTool} accessibilityLabel={description}
    style={[styles.row, !!onTool && styles.interactiveRow,
      !!onTool && compact && styles.interactiveRowCompact, !!onTool && micro && styles.interactiveRowMicro]}>
    ...
    {Array.from({ length: FREE_TOOL_CAPACITY }, (_, index) => {
      ...
      const slotStyle: StyleProp<ViewStyle> = [styles.slot, !!onTool && styles.interactiveSlot,
        !!onTool && compact && styles.interactiveSlotCompact, !!onTool && micro && styles.interactiveSlotMicro, !kind && styles.empty,
      ...
      return <View key={index} testID={`${testID}-${index}`}>
        {kind && onTool ? <Pressable testID={toolTestIDPrefix ? `${toolTestIDPrefix}-${kind}` : `tool-${kind}`} accessibilityRole="button"
          ...
          <ToolIcon kind={kind} size={micro ? 21 : compact ? 23 : 26} color="#244b45" />
...
const styles = StyleSheet.create({
  ...
  interactiveRowCompact: { gap: 6 },
  interactiveRowMicro: { gap: 4 },
  interactiveSlot: { width: 48, height: 48, borderRadius: 16 },
  interactiveSlotCompact: { width: 42, height: 42, borderRadius: 14 },
  interactiveSlotMicro: { width: 38, height: 38, borderRadius: 13 },
