...
The repository is currently scoped to the Milestone 1 technical spike: one
level, one stitch type, one traveler, deterministic movement, immediate retry,
and enough feedback to prove that placing a stitch changes the route. Campaign
screens, persistence, RevenueCat, InsForge, and production content are
...
> performance. Milestone 1 is not device-complete until the checklist in
...
No RevenueCat or InsForge credentials are needed for Milestone 1.
...
native configuration, or the Expo SDK. A JavaScript/TypeScript-only edit only
needs Metro reload.
...
do not use `npm run start:go` or an Expo Go session as Milestone 1 hardware
...
The flow expects an installed app with identifier
...
The flow launches cleanly, places a stitch with a percentage-based drag,
releases the traveler, waits for a terminal result, retries, undoes, places
again, and resets. Maestro proves the control path on the selected installed
target; it does not judge tactile quality or prove physics determinism by
itself.

## Architecture and proof standard

The spike keeps geometry, deformation, and physics in pure TypeScript. The
...
The following are not part of Milestone 1 and should not be inferred from this
spike:
